		...
		User.findOne({ phone_number: phone_number }, function(err, user) {
			...
			if (user) {
				...
				User.create({
					...
					var messageBody = "Hello "+first_name+" "+last_name+", this is Ana Sanchez, recruiter for "+school+" for which you applied to our "+position+" posting. When is the best time to talk?"

					Message.create({
						body : messageBody,
						to : Twilio.standardizePhoneNumber(phone_number),
						from : "+17868081105"
					...
					Twilio.sendMessage(Twilio.standardizePhoneNumber(phone_number), "+17868081105", messageBody);
				});
